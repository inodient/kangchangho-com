			...
			if( queryString.indexOf("?") > -1 ){
				for( var i=0; i<params.length; i++ ){
					if( !isNaN(parseFloat(params[i])) && isFinite(params[i]) ){
						queryString = queryString.replace( "?", "" + encodeParam(params[i]) + "" );
					} else {
						queryString = queryString.replace( "?", "'" + encodeParam(params[i]) + "'" );
					}
				}
				...
				for( var i=0; i<params.length; i++ ){
					if( params[i] instanceof Object ){
						...
						var value = ( encodeParam(params[i]) )[key];
...
function encodeParam( paramString ){
	// logger.debug( paramString );
	// logger.debug( paramString.replace( /'/gi, '&#39;' ) );
